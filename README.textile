...
* Fix the has-many-through-polymorphic issue - not supported by ActiveRecord, so try the other two approaches.
...
* Add abstract error class @IsReviewableError@, which handles logging instead of doing this explicitly.
